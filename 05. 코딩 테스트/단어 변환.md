...
aliases:
  - DFS, BFS
복습 풀이: 251006(O), 251011(O), 251019(O), 251025(O), 251101(O), 251117(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    public int solution(String begin, String target, String[] words) {
        Queue<WordConversion> q = new LinkedList<>();
        q.add(new WordConversion(begin, 0));
        boolean[] visited = new boolean[words.length];
        
        while (!q.isEmpty()) {
            WordConversion current = q.poll();
            
            if (current.word.equals(target)) return current.conversionCount;
            
            for (int i = 0; i < words.length; i++) {
                String value = convert(current.word, words[i]);
                if (visited[i] || value.isEmpty()) continue;
                
                visited[i] = true;
                q.add(new WordConversion(value, current.conversionCount + 1));
            }
        }
    ...
    static class WordConversion {
        private String word;
        private int conversionCount;
        
        public WordConversion(String word, int conversionCount) {
            this.word = word;
