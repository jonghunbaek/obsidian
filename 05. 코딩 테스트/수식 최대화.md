...
aliases:
  - 완전탐색
복습 풀이: 250730(X), 250809(O), 250816(O), 250823(O), 250830(O), 251031()
---
# 1차 풀이
```java
...
class Solution {
    ...
    public long solution(String expression) {
        ...
        for (String order : operationOrder) {
            max = Math.max(max, calculateByOrder(numbers, operations, order));
        }
        
        return max;
    }
    
    private long calculateByOrder(List<String> numbers, List<Character> operations, String order) {
        for (char o : order.toCharArray()) { 
            for (int i = 0; i < operations.size(); i++) {
                if (operations.get(i) == o) continue;
                
                long pre = Long.parseLong(numbers.get(i));
                long post = Long.parseLong(numbers.get(i + 1));
                long result = calculate(pre, post, o);
                numbers.remove(i);
                numbers.remove(i);
        ...
        return Math.abs(Long.parseLong(numbers.get(0)));
    }
