...
aliases:
  - 유니온파인드
복습 풀이: 250916(X), 250921(X), 251004(X), 251011(X), 251017(O), 251024(O), 251107(O)
---
# 1차 풀이
```java
import java.util.*;
...
```
문제는 맞췄지만, 처음 문제를 접근할 때 유니온 파인드를 사용해야 한다는 생각이 전혀 안들었음
어떤 케이스에서 유니온 파인드를 써야할지 명확하게 이해할 필요가 있음

### 💡 유니온-파인드 알고리즘의 핵심 역할
유니온-파인드 알고리즘은 기본적으로 원소들을 여러 개의 집합으로 나누어 관리하며, 다음 두 가지 연산을 효율적으로 수행하는 데 목적이 있습니다.
1. **Find (찾기):** 특정 원소가 속한 집합의 **대표 원소(루트 노드)**를 찾는 연산. 이를 통해 두 원소가 **같은 집합**에 속하는지 빠르게 확인할 수 있습니다.
2. **Union (합치기):** 두 원소가 속한 집합을 **하나로 합치는** 연산.
경로 압축(Path Compression) 및 랭크(Rank)나 크기(Size) 기반의 합치기 최적화를 적용하면 거의 $O(\alpha(N))$의 상수 시간에 가까운 효율적인 연산을 수행할 수 있습니다. ($\alpha$는 아커만 함수의 역함수로, 매우 느리게 증가합니다.)
---
### 💻 코딩 테스트 출제 유형 (핵심 활용 사례)

유니온-파인드 알고리즘은 주로 **연결 관계, 그룹화, 또는 사이클 여부**를 판단해야 하는 문제에서 명확하게 활용됩니다.

| **유형**                | **설명**                                                                                                                   | **핵심 키워드**                  |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------- |
| **1. 최소 신장 트리 (MST)** | **크루스칼(Kruskal) 알고리즘**에서 사용됩니다. 간선을 하나씩 추가할 때, 두 정점이 이미 같은 집합에 속해 있다면 (Find 연산 결과가 같다면) **사이클이 형성**되므로 해당 간선을 추가하지 않습니다. | 크루스칼 알고리즘, 사이클 판별, 최소 비용 연결 |
| **2. 연결 요소 및 그룹 판별**  | 주어진 노드들 간의 관계를 바탕으로 **서로 연결된 그룹(집합)**의 개수를 세거나, 특정 두 노드가 **같은 그룹에 속하는지** 여부를 판단하는 문제입니다.                                 | 그룹화, 네트워크 연결, 친구 관계, 여행 경로  |
| **3. 그래프의 사이클 판별**    | 무방향 그래프에서 간선을 하나씩 추가할 때, 유니온-파인드 연산을 통해 추가하려는 두 노드의 대표 원소가 이미 같다면 **사이클**이 존재한다고 판단합니다. (MST의 원리와 유사)                    | 무방향 그래프, 사이클 여부 확인          |

이러한 문제들을 접했을 때, "어떤 원소들이 서로 연결되어 하나의 집합을 이루는가?"라는 질문에 답해야 한다면 유니온-파인드를 떠올리시면 됩니다.

---
유니온 파인드 알고리즘에 대한 자세한 개념과 구현 방법을 [알고리즘 코딩테스트 핵심이론 강의 - 유니온 파인드](https://www.youtube.com/watch?v=klBh4ZglHYo) 영상에서 확인하실 수 있습니다.