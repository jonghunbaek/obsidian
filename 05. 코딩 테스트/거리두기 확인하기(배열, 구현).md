...
aliases:
  - 배열
복습 풀이: 250714(X), 250721(O), 251013(X), 251021(O), 251025(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    ...
    public int[] solution(String[][] places) {
        int[] results = new int[places.length];
        for (int i = 0; i < places.length; i++) {
            results[i] = isAllInManhattanDistance(places[i]);
        }
        
        return results;
    }
    
    private int isAllInManhattanDistance(String[] target) {
        char[][] place = toCharArray(target);
        for (int y = 0; y < place.length; y++) {
            for (int x = 0; x < place[y].length; x++) {
                ...
                for (int d = 0; d < 4; d++) {
                    ...
                    if (isOutOfRange(nx, ny, place)) continue;
                    if (place[ny][nx] == 'P') return 0;
                    if (isInParticipantNearBy(nx, ny, x, y, place)) return 0;
                }
            }
        }
    ...
    private char[][] toCharArray(String[] target) {
        char[][] result = new char[target.length][target[0].length()];
        for (int i = 0; i < target.length; i++) {
            result[i] = target[i].toCharArray();
        }
        
        return result;
    }
    
    private boolean isInParticipantNearBy(int x, int y, int preX, int preY, char[][] place) {
        for (int d = 0; d < 4; d++) {
            int nx = dx[d] + x;
            int ny = dy[d] + y;
    ...
    private boolean isOutOfRange(int x, int y, char[][] place) {
        return x < 0 || x >= place[0].length || y < 0 || y >= place.length || place[y][x] == 'X';
    }
}
```