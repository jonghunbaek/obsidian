...
aliases:
  - 재귀
복습 풀이: 250812(X), 250818(O), 250823(O), 250830(O), 250906(O), 251027(O), 251101(O)
---
# 1차 풀이
```java
import java.util.*;
...
class Solution {
    public String[] solution(String[] orders, int[] course) {
        ...
        for (int size : course) {
            ...
            for (String order : orders) {
                countMenu(0, new ArrayList<>(),order, size, menuCounts);
            }
            
            int max = menuCounts.values().stream()
                    .mapToInt(Integer::intValue)
                    .max()
                    .orElse(0);

            if (max < 2) continue;
            List<String> menus = menuCounts.entrySet().stream()
                .filter(entry -> entry.getValue() == max)
                .map(entry -> entry.getKey())
                .collect(Collectors.toList());
    ...
    private void countMenu(int start, List<String> target, String order, int size, Map<String, Integer> menusCounts) {
        if (target.size() == size) {
            String value = target.stream()
                .sorted()
                .collect(Collectors.joining());
            
            menusCounts.put(value, menusCounts.getOrDefault(value, 0) + 1);
            return;
        }
        
        for (int i = start; i < order.length(); i++) {
            target.add(order.substring(i, i + 1));
            countMenu(i + 1, target, order, size, menusCounts);
            target.remove(order.substring(i, i + 1));
        }
    }
}
```
- 이중 반복문에서 외부 반복문에서 수행해야 하는 로직을 내부 반복문에서 수행해 실패, 디버깅 시간이 오래걸림