...
class Solution {
    public int solution(int n, int[][] q, int[] ans) {
        return count(0, 1, new int[5], n, q, ans);
    }
    
    private int count(int depth, int idx, int[] cipher, int n, int[][] q, int[] ans) {
        if (depth == 5) {
            if (isCorrect(cipher, q, ans)) return 1;
            return 0;
        }
        ...
        for (int i = idx; i <= n; i++) {
            cipher[depth] = i;
            result += count(depth + 1, i + 1, cipher, n, q, ans);
        }
        
        return result;
    }
    
    private boolean isCorrect(int[] cipher, int[][] q, int[] ans) {
        for (int i = 0; i < q.length; i++) {
            int count = 0;
            for (int j = 0; j < cipher.length; j++) {
                for (int k = 0; k < q[i].length; k++) {
                    if (cipher[j] == q[i][k]) count++;
                }
            }
...
```
지난번 최초 풀이에서 시간복잡도가 발생한 원인 다시한번 확인하기.