...
aliases:
  - 문자열, 구현
복습 풀이: 250723(X), 250727(X), 250802(X), 250809(O), 250816(X), 250823(O), 250830(O), 251017(O), 260120(O), 260202(O), 260207(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    public String solution(String new_id) {
        String target = new_id.toLowerCase();           // 1단계
        target = target.replaceAll("[^a-z0-9-_.]", ""); // 2단계
        target = target.replaceAll("\\.+", ".");          // 3단계
        target = target.replaceAll("^\\.+|\\.+$", "");      // 4단계
        target = target.isBlank() ? "a" : target;       // 5단계
        if (target.length() >= 16) {                    // 6단계
            target = target.substring(0, 15);
        }
        
        target = target.replaceAll("\\.+$", "");
        
        if (target.length() <= 2) {                     // 7단계
            target = target + String.valueOf(target.charAt(target.length() - 1)).repeat(3 - target.length());
        }
...
여전히 정규 표현식에 익숙하지 않음 정규 표현식이 익숙해질 때까지 계속 풀기
^는 문자열 시작, $는 문자열 끝
...
class Solution {
    public String solution(String new_id) {
        ...
        target = target.replaceAll("[^a-z0-9-_.]", "");
        target = target.replaceAll("\\.+", "."); // 3단계
        target = target.replaceAll("^\\.+|\\.+$", ""); // 4단계
        target = target.isBlank() ? "a" : target; // 5단계
        target = target.length() >= 16 ? target.substring(0, 15) : target;
        target = target.replaceAll("\\.+$", "");
        
        if (target.length() <= 2) {
            target = target + target.substring(target.length() - 1).repeat(3 - target.length());
        }
...
class Solution {
    public String solution(String new_id) {
        ...
        target = target.replaceAll("\\.+", ".");
        target = target.replaceAll("^\\.+|\\.+$", "");
        target = target.isEmpty() ? "a" : target;
        if (target.length() >= 16) {
            target = target.substring(0, 15);
            target = target.replaceAll("\\.+$", "");
        }
...
class Solution {
    public String solution(String new_id) {
        String target = new_id.toLowerCase();
        target = target.replaceAll("[^0-9a-z-_.]", "");
        target = target.replaceAll("\\.+", ".");
        target = target.replaceAll("^\\.+|\\.+$", "");
        target = target.isEmpty() ? "a" : target;
        if (target.length() >= 16) target = target.substring(0, 15);
        target = target.replaceAll("\\.$", "");
        if (target.length() < 3) target += target.substring(target.length() - 1, target.length()).repeat(3 - target.length());
...
class Solution {
    public String solution(String new_id) {
        ...
        if (target.length() < 3) {
            target += target.substring(target.length() - 1, target.length()).repeat(3 - target.length());
        }
