...
date: 2025-11-19
aliases:
복습 풀이: 251119(O), 251126(O), 251209(O)
---
# 1차 풀이
```java
import java.util.*;
...
class Solution {
    public int[] solution(int[] fees, String[] records) {
        Map<String, List<Integer>> timeByCar = new HashMap<>();        
        for (String record : records) {
            String[] split = record.split(" ");
            int value = toMinute(split[0]);
            String carNumber = split[1];
            
            List<Integer> times = timeByCar.getOrDefault(carNumber, new ArrayList<>());
            times.add(value);
            timeByCar.put(carNumber, times);
        }    
        
        return timeByCar.entrySet().stream()
            .sorted((e1, e2) -> e1.getKey().compareTo(e2.getKey()))
            .mapToInt(entry -> calculateTotalFee(entry.getValue(), fees))
            .toArray();
    }
    ...
    private int calculateTotalFee(List<Integer> times, int[] fees) {
         int defaultTime = fees[0];
        int defaultFee = fees[1];
        int perMinute = fees[2];
        int perFee = fees[3];
        ...
        for (int i = 0; i < times.size(); i += 2) {
            if (i + 1 == times.size()) {
                totalMinutes += (23 * 60 + 59) - times.get(i);
            } else {
        ...
        int extraMinutes = totalMinutes - defaultTime;
        int extraFee = (int) Math.ceil((double) extraMinutes / perMinute) * perFee;
