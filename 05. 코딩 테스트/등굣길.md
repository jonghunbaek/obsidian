...
aliases:
  - dp
복습 풀이: 250826(X), 250901(X), 250907(X), 250913(O), 250920(O), 251030(X), 251102(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    ...
    private int dfs(int x, int y, int[][] map) {
        if (x >= map[0].length || y >= map.length) return 0;
        if (x == map[0].length - 1 && y == map.length - 1) {
            map[y][x] = 1;
            return 1;
        }
        ...
        map[y][x] += (dfs(x + 1, y, map) + dfs(x, y + 1, map)) % 1000000007;
        return map[y][x];
    }
}
```