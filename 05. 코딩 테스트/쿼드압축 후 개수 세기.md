...
aliases:
  - 재귀
복습 풀이: 250724(X), 250727(O), 250802(O), 250809(O), 250816(O), 251017(O), 251021(O), 251025(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    ...
    private int[] count(int x, int y, int size, int[][] arr) {    
        for (int i = y; i < y + size; i++) {
            for (int j = x; j < x + size; j++) {
                if (arr[y][x] == arr[i][j]) continue;
                
                int[] first = count(x, y, size / 2, arr);
                int[] second = count(x + size / 2, y, size / 2, arr);
                int[] third = count(x, y + size / 2, size / 2, arr);
                int[] fourth = count(x + size / 2, y + size / 2, size / 2, arr);
                
                return add(first, second, third, fourth);
            }
        }
        
        return arr[y][x] == 1 ? new int[] {0, 1} : new int[] {1, 0};
    }
    
    private int[] add(int[] first, int[] second, int[] third, int[] fourth) {
        return new int[] {
...
```
종료 조건을 계속 size == 1이라고 생각하고 접근하게됨
문제를 꼼꼼히 읽어보면 종료 조건은 배열의 모든 요소가 같은 경우다. 
    ...
    private int[] count(int x, int y, int size, int[][] arr) {
        for (int i = y; i < y + size; i++) {
            for (int j = x; j < x + size; j++) {
                ...
                int half = size / 2;
                int[] first = count(x, y, half, arr);
                int[] second = count(x + half, y, half, arr);
                int[] third = count(x, y + half, half, arr);
                int[] fourth = count(x + half, y + half, half, arr);
