...
tags:
date: 2025-12-31
복습 풀이: 251231(X), 260105(O), 260111(O)
---
# 1차 풀이
```java
// 문자열, 구현, 해시
...
class Solution {
    public int[] solution(String today, String[] terms, String[] privacies) {
        int targetDay = toDay(today);
        Map<String, Integer> expirationByTerm = new HashMap<>();
        for (String term : terms) {
            expirationByTerm.put(term.split(" ")[0], Integer.parseInt(term.split(" ")[1]) * 28);
        }
        
        List<Integer> results = new ArrayList<>();
        for (int i = 0; i < privacies.length; i++) {
            int termDay = toDay(privacies[i].split(" ")[0]);
            String term = privacies[i].split(" ")[1];
            int expiration = expirationByTerm.get(term);
            
            if (termDay + expiration <= targetDay) results.add(i + 1);
        }
