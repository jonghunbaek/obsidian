...
aliases:
  - 큐
복습 풀이: 250903(X), 250907(X), 250913(X), 250920(O), 251003(O), 251105(O), 251111(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        while (truckIdx < truck_weights.length) {
            ...
            if (totalWeight + truck_weights[truckIdx] > weight) {
                bridge.offer(0);
            } else {
                int pushed = truck_weights[truckIdx];
                bridge.offer(pushed);
                truckIdx++;
                totalWeight += pushed;
            }
...
class Solution {
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        int idx = 0;
        while(idx < truck_weights.length) {
            int leave = bridge.poll();
            totalWeight -= leave;

            if (totalWeight + truck_weights[idx] <= weight) {
                int current = truck_weights[idx];
...
class Solution {
    public int solution(int bridgeLength, int weight, int[] truckWeights) {
        ...
        while (idx < truckWeights.length) {
            ...
            if (totalWeight + truckWeights[idx] > weight) {
                bridge.offer(0);
            } else {
