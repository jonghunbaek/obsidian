...
aliases:
  - 유니온파인드
복습 풀이: 250917(X), 250921(X), 251004(X), 251011(O), 251017(O), 251024(O), 251112(O), 251115(O)
---
# 1차 풀이
```java
import java.util.*;
...
class Solution {
    public long[] solution(long k, long[] room_number) {
        ...
        for (int i = 0; i < room_number.length; i++) {
            results[i] = findRoom(room_number[i], rooms);
        }
        
        return results;
    }
    
    private long findRoom(long currentRoom, Map<Long, Long> rooms) {
        if (!rooms.containsKey(currentRoom)) {
            rooms.put(currentRoom, currentRoom + 1);
            return currentRoom;
        }
        
        long nextRoom = findRoom(rooms.get(currentRoom), rooms);
        rooms.put(currentRoom, nextRoom);
        return nextRoom;
    }
}
```