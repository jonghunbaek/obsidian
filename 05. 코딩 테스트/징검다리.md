...
aliases:
  - 이진탐색
복습 풀이: 250819(X), 250824(X), 250901(X), 250906(X), 250913(O), 250919(O), 250926(O), 251029(O), 251101(O)
---
# 1차 풀이
```java
...
class Solution {
    ...
    private boolean isValid(int mid, int[] rocks, int n) {
        ...
        return removed <= n; // 제거 개수가 n개 이하면 더 큰 기준이 답이 될 수 있는지 확인하기 위함
    }
}
```
...
class Solution {
    ...
    private boolean isValid(int minimum, int[] rocks, int n) {
        int removed = 0;
        int last = 0;
        for (int rock : rocks) {
            if (rock - last < minimum) {
                removed++;
            } else {
                last = rock;
            }
...
class Solution {
    public int solution(int distance, int[] rocks, int n) {
        ...
        while(start <= end) {
            ...
            if (isValid(mid, rocks, n)) { // 정답이 될 수 있는 값 중, 최댓값을 찾기 위해 시작값을 증가
                answer = mid;
                start = mid + 1;
            } else {
                end = mid - 1;
    ...
    private boolean isValid(int minimumDistance, int[] rocks, int n) {
        int lastRock = 0;
        int removed = 0;
        for (int rock : rocks) {
            if (rock - lastRock >= minimumDistance) {
                lastRock = rock;
            } else {
                removed++;
            }
