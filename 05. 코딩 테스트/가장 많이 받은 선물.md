...
class Solution {
    public int solution(String[] friends, String[] gifts) {
        ...
        for (String gift : gifts) {
            ...
            int fromIdx = indexes.get(from);
            int toIdx = indexes.get(to);
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            for (int j = 0; j < counts[i].length; j++) {
                if (i == j) continue;
                if (counts[i][j] > counts[j][i]) received++;
                if (counts[i][j] == counts[j][i] && giftIndex(i, counts) > giftIndex(j, counts)) received++;
            }
            
            max = Math.max(max, received);
        }
        
        return max;
    }
    
    private int giftIndex(int target, int[][] counts) {
        int received = 0;
        int gived = 0;
        for (int i = 0; i < counts.length; i ++) {
            received += counts[i][target];
            gived += counts[target][i];
        }
        
        return gived - received;
    }
}
```