...
aliases:
  - 재귀, DFS
복습 풀이: 250924(O), 251006(O), 251011(O), 251019(O), 251025(O), 251114(O)
---
# 1차 풀이
```java
import java.util.*;
...
class Solution {
    public int solution(int[] numbers, int target) {
        return countMatchCase("", target, numbers);
    }
    
    private int countMatchCase(String operations, int target, int[] numbers) {
        if(operations.length() == numbers.length) {
            if (sum(operations, numbers) == target) return 1;
            return 0;
        }
        
        return countMatchCase(operations + "+", target, numbers) + countMatchCase(operations + "-", target, numbers);
    }
    
    private int sum(String operations, int[] numbers) {
        int result = 0;
        for (int i = 0; i < numbers.length; i++) {
...
class Solution {
    ...
    private int count(String target, int[] numbers, int targetNumber) {
        if (target.length() == numbers.length) return sum(target, numbers) == targetNumber ? 1 : 0;
        
        return count(target + "+", numbers, targetNumber) + count(target + "-", numbers, targetNumber);
    }
    
    private int sum(String target, int[] numbers) {
        int result = 0;
        for (int i = 0; i < numbers.length; i++) {
            if (target.charAt(i) == '+') {
                result += numbers[i];
