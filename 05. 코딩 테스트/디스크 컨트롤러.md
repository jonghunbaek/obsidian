...
aliases:
  - 우선순위큐
복습 풀이: 250911(X), 250915(X), 250920(O), 251004(O), 251107(X), 251111(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    public int solution(int[][] jobs) {
        Task[] temp = new Task[jobs.length];
        for (int i = 0; i < jobs.length; i++) {
            temp[i] = new Task(jobs[i][0], jobs[i][1]);
        }
        Arrays.sort(temp, (t1, t2) -> t1.requestedAt - t2.requestedAt);
        Queue<Task> tasks = new LinkedList<>();
        for (Task t : temp) {
        ...
        while(!tasks.isEmpty() || !waiting.isEmpty()) {
            ...
            Task currentTask = waiting.poll();
            currentTime += currentTask.duration;
            totalReturnTime += currentTime - currentTask.requestedAt;
        }
