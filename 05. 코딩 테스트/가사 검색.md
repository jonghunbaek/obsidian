...
aliases:
  - 트라이
복습 풀이: 250918(X), 250922(X), 251004(X), 251011(O), 251019(O), 251024(X), 251101(O), 251113(O), 251117(O)
---
# 1차 풀이
```java
class Solution {
...
class Solution {
    ...
    static class Trie {
        ...
        private void insert(String target) {
            int length = target.length();
            count.put(length, count.getOrDefault(length, 0) + 1);
            Trie node = this;
            for (char c : target.toCharArray()) {
        ...
        private int search(String target) {
            ...
            for (char c : target.toCharArray()) {
                ...
                if (c == '?') return node.count.getOrDefault(length, 0);
...
```
search()에서 분기 처리 순서가 잘못되어 틀림
이 또한 답안을 거의 외우다시피 한것이 틀린 주요 원인
꼼꼼히 분석해가며 풀어보기.
    ...
    static class Trie {
        ...
        private void insert(String word) {
            ...
            for (char c : word.toCharArray()) {
                ...
                node = node.children.get(c);
                node.counts.put(length, node.counts.getOrDefault(length, 0) + 1);
            }
        }
        ...
        private int search(String word) {
            ...
            for (char c : word.toCharArray()) {
                if (c == '?') return node.counts.getOrDefault(length, 0);
                if (!node.children.containsKey(c)) return 0;
                
                node = node.children.get(c);
            }
            
            return node.counts.getOrDefault(length, 0);
        }
    }
}
```
