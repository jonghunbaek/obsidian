...
aliases:
  - 완전탐색
복습 풀이: 250801(X), 250817(X), 250823(O), 250830(O), 250906(O), 250913(O), 251022(O), 251026(O)
---
# 1차 풀이
```java
// 재귀 형태로 구현  
...
class Solution {
    public int solution(String[] user_id, String[] banned_id) {
        ...
        countSetCases(0, set, user_id, banned_id, results);
        
        return results.size();
    }
    
    private void countSetCases(int idx, Set<String> set, String[] usersId, String[] bannedsId, Set<Set<String>> results) {
        if (set.size() == bannedsId.length) {
            results.add(new HashSet<>(set));
            return;
        }
        
        for (int i = 0; i < usersId.length; i++) {
            String pattern = bannedsId[idx].replace("*", ".");
            if (set.contains(usersId[i]) || !usersId[i].matches(pattern)) continue;
            
            set.add(usersId[i]);
            countSetCases(idx + 1, set, usersId, bannedsId, results);
            set.remove(usersId[i]);
        }
    }
}
```