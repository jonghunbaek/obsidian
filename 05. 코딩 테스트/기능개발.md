...
aliases:
  - 큐
복습 풀이: 250902(X), 250907(O), 250913(O), 250920(O), 251104(O)
---
# 1차 풀이
### 최초 풀이
```java
...
```
어찌저찌 맞췄으나 완전 어거지로 맞춘 느낌
이전 풀이를 보니 굳이 Stack을 안써도 됐음.
이전에 풀었던 경험들이 오히려 독이된 느낌
- 무조건 Stack을 사용해야만 한다고 착각함

또한 다음 부분에서도 실수가 있었음
- Math.ceil()할 때, int 타입이면 이미 버림이 발생한 상태에서 수행하게됨
