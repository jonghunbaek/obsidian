...
tags:
  ...
  - 우선순위큐
date: 2026-01-22
복습 풀이: 260122(X), 260205(O), 260207(O), 260214(O), 260224(O)
---
# 1차 풀이
```java
// 구현, 정렬
...
class Solution {
    public int[] solution(int N, int[] stages) {
        ...
        for (int i = 1; i <= N; i++) {
            int count = counts.getOrDefault(i, 0);
            if (count == 0 || challengers == 0) {
                results.add(new Result(i, 0.0));
                continue;
            }
            
            double failRate = (double) count / challengers;
            results.add(new Result(i, failRate));
            challengers -= count;
        }
        
        return results.stream()
            .sorted((r1, r2) -> {
                if (r1.failRate == r2.failRate) return r1.stage - r2.stage;
                return Double.compare(r2.failRate, r1.failRate);
            })
            .mapToInt(r -> r.stage)
            .toArray();
