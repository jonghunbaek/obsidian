...
aliases:
  - 이진탐색
복습 풀이: 250815(X), 250818(X), 250823(X), 250830(X), 250906(O), 250913(O), 250919(O), 251028(X), 251101(O), 251110(O)
---
# 1차 풀이
### 처음 풀이
```java
...
# 5차 풀이
```java
// 1. 모든 조건 경우의 수를 만들어 Map에 키로 저장
//  1-1. 값은 List형태로 코테 점수를 정렬하여 저장
// 2. query 배열을 순회하며 조건에 맞는 인원수를 파악
class Solution {
    public int[] solution(String[] info, String[] query) {
        List<Integer> results = new ArrayList<>();
        Map<String, List<Integer>> scoresByCondition = init(info);
        for (String condition : query) {
            ...
            List<Integer> scores = scoresByCondition.get(key);
            int score = Integer.valueOf(splited[7]);
            int idx = binarySearch(score, scores);
            results.add(scores.size() - idx);
        }
    ...
    private Map<String, List<Integer>> init(String[] info) {
        Map<String, List<Integer>> scoresByCondition = new HashMap<>();
        for (String value : info) {
            ...
            combineConditions(0, "", conditions, score, scoresByCondition);
        }
        
        scoresByCondition.values().stream()
            .forEach(value -> Collections.sort(value));
        
        return scoresByCondition;
    }
    
    private void combineConditions(int idx, String target, String[] conditions, int score, Map<String, List<Integer>> scoresByCondition) {
        if (idx == conditions.length) {
            List<Integer> scores = scoresByCondition.getOrDefault(target, new ArrayList<>());
            scores.add(score);
            scoresByCondition.put(target, scores);
            return;
        }
        
        combineConditions(idx + 1, target + conditions[idx], conditions, score, scoresByCondition);
        combineConditions(idx + 1, target + "-", conditions, score, scoresByCondition);
    }
...
```
binarySearch에서 조건을 scores.get(mid) >= score로 하지 않으면 틀림.
이 조건에 대한 이유를 명확히 이해해야 함

이 문제에선 **지정된 점수 이상에 해당되는 첫 번째 인덱스를 찾아야함**
때문에 scores.get(mid) > score로 해버리면 초과되는 첫 번째 인데스를 찾는 것이 됨

# 6차 풀이
```java
// 1. info를 Map형태로 변환하며, 모든 경우의 수를 구해 값으로 점수 리스트 저장
// 2. query를 순회하며, 조건에 일치하는 사용자를 탐색
import java.util.*;

class Solution {
    public int[] solution(String[] info, String[] query) {
        Map<String, List<Integer>> scoresByCondition = sortScoreByCondition(info);    
        List<Integer> results = new ArrayList<>();
        for (String value : query) {
            String[] split = value.split(" ");
            String condition = split[0] + split[2] + split[4] + split[6];
            int score = Integer.parseInt(split[7]);
            
            if (!scoresByCondition.containsKey(condition)) {
                results.add(0);
                continue;
            }
            
            List<Integer> scores = scoresByCondition.get(condition);
            int idx = binarySearch(score, scores);
    ...
    private Map<String, List<Integer>> sortScoreByCondition(String[] info) {
        Map<String, List<Integer>> scoresByCondition = new HashMap<>();
        for (String value : info) {
            String[] split = value.split(" ");
            String[] conditions = Arrays.copyOfRange(split, 0, 4);
            int score = Integer.parseInt(split[4]);
            createCases(0, "", conditions, score, scoresByCondition);
        }
        
        scoresByCondition.values().forEach(value -> Collections.sort(value));
        
        return scoresByCondition;
    }
    
    private void createCases(int depth, String target, String[] conditions, int score, Map<String, List<Integer>> map) {
        if (depth == conditions.length) {                
            List<Integer> scores = map.getOrDefault(target, new ArrayList<>());
            scores.add(score);
            map.put(target, scores);
            return;
        }
        
        createCases(depth + 1, target + conditions[depth], conditions, score, map);
        createCases(depth + 1, target + "-", conditions, score, map);
    }
    ...
    private int binarySearch(int score, List<Integer> scores) {
        ...
        while (start <= end) {
            ...
            if (score > scores.get(mid)) { // score >= scores.get()으로 하면 문제 조건에 맞지 않음 
                start = mid + 1;
            } else {
                end = mid - 1;
            }
...
```
지난 주에 헷갈렸던 부분을 또 헷갈림
다시 정리하자면 아래와 같음
- 문제에서 score 이상이되는 모든 점수를 찾아야 함
- 그러므로 기준은 score가 되고, scores.get()이 내가 찾는 그 인덱스가 되는 거임
- 따라서 scores.get() >= score로 찾아야 내가 찾는 조건에 맞는 인덱스를 구할 수 있음

# 7차 풀이
```java
// 1. info를 순회하며 모든 경우의 수를 Map형태로 저장 key는 조건, value는 코테 점수
// 2. query를 순회하며 조건 기준에 부합하는 인원수 계산

import java.util.*;

class Solution {
    public int[] solution(String[] info, String[] query) {
        Map<String, List<Integer>> map = createAllCases(info);
        ...
        for (int i = 0; i < query.length; i++) {
            ...
            int idx = binarySearch(score, scores);
            results[i] = scores.size() - idx;
        }
        
        return results;
    }
    
    private Map<String, List<Integer>> createAllCases(String[] info) {
        Map<String, List<Integer>> map = new HashMap<>();
        for (String value : info) {
            String[] split = value.split(" ");
            String[] conditions = Arrays.copyOfRange(split, 0, 4);
            int score = Integer.parseInt(split[4]);
            createCase("", 0, conditions, score, map);
        }
    ...
    private void createCase(String target, int idx, String[] conditions, int score, Map<String, List<Integer>> map) {
        if (idx == conditions.length) {
            List<Integer> scores = map.getOrDefault(target, new ArrayList<>());
            scores.add(score);
            map.put(target, scores);
        ...
        createCase(target + conditions[idx], idx + 1, conditions, score, map);
        createCase(target + "-", idx + 1, conditions, score, map);
    }
...
# 8차 풀이
```java
// 1. info를 순회하며, 각 조건을 '-'로 대체한 모든 경우의수를 Map에 저장(key : 조건, value : 코테 점수 List)
// 2. query를 순회하며, 조건에 일치하는 수험생이 몇명인지 검색(100,000 * 50,000이므로 시간초과니 이분 탐색을 활용)

import java.util.*;

class Solution {
    public int[] solution(String[] info, String[] query) {
        Map<String, List<Integer>> scoresByCondition = groupScoresByCondition(info);
        
        return countSatisfyingConditions(query, scoresByCondition);
    }
    
    private Map<String, List<Integer>> groupScoresByCondition(String[] info) {
        Map<String, List<Integer>> scoresByCondition = new HashMap<>();
        for (String i : info) {
            ...
            groupByCondition(0, "", conditions, score, scoresByCondition);
        }
        
        scoresByCondition.values().forEach(Collections::sort);
        
        return scoresByCondition;
    }
    
    private void groupByCondition(int depth, String target, String[] conditions, int score, Map<String, List<Integer>> map) {
        if (depth == conditions.length) {
            List<Integer> scores = map.getOrDefault(target, new ArrayList<>());
            scores.add(score);
            map.put(target, scores);
        ...
        groupByCondition(depth + 1, target + "-", conditions, score, map);
        groupByCondition(depth + 1, target + conditions[depth], conditions, score, map);
    }
    
    private int[] countSatisfyingConditions(String[] query, Map<String, List<Integer>> scoresByCondition) {
        int[] results = new int[query.length];
        ...
        for (int i = 0; i < query.length; i++) {
            ...
            if (!scoresByCondition.containsKey(condition)) continue;
            List<Integer> scores = scoresByCondition.get(condition);
            
            results[i] = scores.size() - binarySearch(score, scores);
        }
    ...
    private int binarySearch(int score, List<Integer> scores) {
        ...
        while (start <= end) {
            ...
            if (scores.get(mid) >= score) { // mid 위치의 점수가 타겟 점수 이상이면 더 작은 값이 있는지 확인
                end = mid - 1;
            } else {
                start = mid + 1;
            }
...
```
- 오랜만에 이분탐색을 푸니 조금 어색함
	- 템플릿을 좀 더 가다듬어 문서로 정리하기
- scoresByCondition의 값을 정렬하는 것에 혼동이 있었음
	- `List<Integer>`으로 반환되는줄 알았지만 실제는 `List<List<Integer>>`로 반환
- scoresByCondition에서 값을 꺼낼 때, 키가 없는 경우에 대한 예외 처리를 하지 못함

재귀, 이분탐색이 복합적으로 들어가고 구현 또한 들어가 좋은 문제라 생각이 든다.
...
class Solution {
    public int[] solution(String[] info, String[] query) {
        ...
        for(String i : info) {
            ...
            groupScoresByCondition(0, "", conditions, score, scoresByCondition);
        }
    ...
    private void groupScoresByCondition(int depth, String target, String[] conditions, int score, Map<String, List<Integer>> map) {
        if (depth == conditions.length) {
            List<Integer> scores = map.getOrDefault(target, new ArrayList<>());
            scores.add(score);
            map.put(target, scores);
        ...
        groupScoresByCondition(depth + 1, target + conditions[depth], conditions, score, map);
        groupScoresByCondition(depth + 1, target + "-", conditions, score, map);
    }
...
class Solution {
    public int[] solution(String[] info, String[] query) {
        Map<String, List<Integer>> scoresByCondition = groupByCondition(info);
    ...
    private Map<String, List<Integer>> groupByCondition(String[] info) {
        Map<String, List<Integer>> scoresByCondition = new HashMap<>();
        for (String i : info) {
            String[] split = i.split(" ");
            String[] conditions = Arrays.copyOfRange(split, 0, 4);
    ...
    private void combineConditions(int idx, String condition, String[] conditions, int score, Map<String, List<Integer>> map) {
        if (idx == conditions.length) {
            List<Integer> scores = map.getOrDefault(condition, new ArrayList<>());
            scores.add(score);
            map.put(condition, scores);
            return; 
        }
        
        combineConditions(idx + 1, condition + conditions[idx], conditions, score, map);
        combineConditions(idx + 1, condition + "-", conditions, score, map);
    }
