...
aliases:
  - 투포인터
복습 풀이: 250915(X), 250920(X), 251004(O), 251011(O), 251017(O), 251024(O), 251107(O), 251111(O), 251115(O)
---
# 1차 풀이
```java
...
class Solution {
    public int[] solution(String[] gems) {
        ...
        int start = 0;
        int end = 1;
        counts.put(gems[start], 1);
        while (end <= gems.length) {
            if (size == counts.size()) {
                putResult(start + 1, end, results);
                
                String gem = gems[start];
                int count = counts.getOrDefault(gem, 0);
                
                if (count < 2) {
                    counts.remove(gem);
                } else {
                    counts.put(gem, count - 1);
                }
    ...
    private void putResult(int start, int end, int[] results) {
        int origin = results[1] - results[0];
        int target = end - start;
...
```
여러번 풀며 느끼지만 다음 부분들은 문제를 처음봤다면 계속 헷갈렸을 것 같다.
- while문의 반복 조건
- start와 end의 초기값
이러한 부분들을 고려해서 정리하며 풀어보기
