...
tags:
  ...
  - 구현
date: 2025-12-11
복습 풀이: 251211(O), 251219(O), 251229(X), 260104(X), 260110(X), 260117(O), 260130(O), 260214(O), 260224()
---
# 1차 풀이
```java
// 1. 단순 배열 문제
...
1. 타겟 대상의 상대 위치를 구하기 -> `(num - 1) % w`
	1. 위치를 구하는 것이므로 -1을 해주어 인덱스를 확인해야 함
2. 상대 위치로부터 끝위치까지의 거리 구하기 -> `w - (상대 위치) - 1`
	1. 끝 위치까지의 거리를 구하기 위해 추가적으로 -1을 해줌
3. 상자를 쌓으면서 좌 -> 우로 방향이 바뀌므로 x 2 + 1로 다음 줄로 이동
	1. x2는 왕복을, +1은 다음 줄로 이동하는 값
# 4차 풀이
```java
class Solution {
    public int solution(int n, int w, int num) {
