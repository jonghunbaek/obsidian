...
aliases:
  - dp
복습 풀이: 250825(O), 250830(O), 250906(O), 250913(O), 250919(O), 251003(O), 251029(O), 251101(O), 251110(X), 251115(O)
---
# 1차 풀이
```java
...
class Solution {
    public int solution(int[][] triangle) {
        ...
        return maximize(0, 0, triangle, mem);
    }
    
    private int maximize(int x, int y, int[][] triangle, int[][] mem) {
        if (x >= mem[0].length || y >= mem.length) return 0;
        if (mem[y][x] != -1) return mem[y][x];
        if (y == mem.length - 1) {
            mem[y][x] = triangle[y][x];
            return mem[y][x];
        ...
        mem[y][x] += Math.max(maximize(x + 1, y + 1, triangle, mem), maximize(x, y + 1, triangle, mem)) + triangle[y][x];
...
이 부분이 잘못되어서 계속 틀렸다. -1로 mem을 초기화했기 때문에 +=로 누적하니 답이 틀린것.
완벽히 파악했다 생각했는데, 예상치 못한 곳에서 틀렸다. 
