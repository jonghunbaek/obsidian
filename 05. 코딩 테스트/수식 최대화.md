...
aliases:
  - 완전탐색
복습 풀이: 250730(X), 250809(O), 250816(O), 250823(O), 250830(O), 251031(O), 251103(O)
---
# 1차 풀이
```java
...
class Solution {
    ...
    public long solution(String expression) {
        ...
        for (String order : ORDERS) {
            ...
            long value = calculateByOrder(order, copy);
    ...
    private long calculateByOrder(String order, List<String> tokens) {
        for (char operation : order.toCharArray()) {
            for (int i = 0; i < tokens.size(); i++) {
                if (String.valueOf(operation).equals(tokens.get(i))) {
                    long pre = Long.parseLong(tokens.get(i - 1));
                    long post = Long.parseLong(tokens.get(i + 1));
                    long value = calculateByOperation(pre, post, operation);
    ...
    private long calculateByOperation(long pre, long post, char operation) {
        if (operation == '+') {
            return pre + post;
        } else if (operation == '-') {
            return pre - post;
...
```
StringTokenizer를 이용해서 풀어봤음.
어색하지만 원리는 같기 때문에 어렵지 않았으나, 아직 어색함 마지막 한번더 풀어보기
...
class Solution {
    ...
    public long solution(String expression) {
        ...
        long result = 0;
        for (String order : ORDERS) {
            long value = calculateByOrder(new ArrayList<>(tokens), order);
            result = Math.max(Math.abs(value), result);
        }
    ...
    private long calculateByOrder(List<String> tokens, String order) {
        for (int i = 0; i < order.length(); i++) {
            char operation = order.charAt(i);
            for (int j = 0; j < tokens.size(); j++) {
                if (!tokens.get(j).equals(String.valueOf(operation))) {
                    continue;
                }
                
                long pre = Long.parseLong(tokens.get(j - 1));
                long post = Long.parseLong(tokens.get(j + 1));
                long result = calculateByOperation(pre, post, operation);
                
                tokens.remove(j - 1);
                tokens.remove(j - 1);
                tokens.remove(j - 1);
                tokens.add(j - 1, String.valueOf(result));
                j -= 2;
            }
        }
...
```
반복문 돌 때, tokens의 원소를 넣고 빼는 부분이 어색하지만 충분히 커버할 수 있음

# 6차 풀이
```java
// 1. 연산자 우선 순위별 값 구하기
// 2. 절대값을 구하여 가장 큰 값을 반환
import java.util.*;

class Solution {
    
    private String[] operationOrder = new String[] {
        "+-*", "+*-", "-+*", "-*+", "*+-", "*-+"
    };
    ...
    public long solution(String expression) {
        ...
        long max = 0;
        for (String order : operationOrder) {
            max = Math.max(max, calculateByOrder(new ArrayList<>(tokens), order));
        }
    ...
    private long calculateByOrder(List<String> tokens, String order) {
        for (char op : order.toCharArray()) { 
            ...
            for (int i = 0; i < tokens.size(); i++) {
                if (!tokens.get(i).equals(o)) continue;
                
                long pre = Long.parseLong(tokens.get(i - 1));
                long post = Long.parseLong(tokens.get(i + 1));
                long result = calculate(pre, post, op);
                tokens.remove(i - 1);
                tokens.remove(i - 1);
                tokens.remove(i - 1);
    ...
    private long calculate(long pre, long post, char operation) {
        if (operation == '+') return pre + post;
        if (operation == '-') return pre - post;
        return pre * post;
    }
}
```
문제가 어렵진 않지만 세부구현에서 시간이 오래걸림
- 연산자 피연산자를 두 개의 List에 담아 풀다가 실패
	- 이전 풀이를 보니 가능한 풀이임
- 두 개를 합쳐서 푸는데 List의 요소를 제거하는 부분에서 오래걸림
...
class Solution {
    ...
    private String[] operationOrders = new String[] {
        "+-*", "+*-", "-+*", "-*+", "*+-", "*-+"
    };
    ...
    public long solution(String expression) {
        ...
        long max = 0;
        for (String order : operationOrders) {
            max = Math.max(max, calculateByOperation(new ArrayList<>(tokens), order));
        }
        
        return max;
    }
    
    private List<String> toTokens(String target) {
    ...
    private long calculateByOperation(List<String> tokens, String order) {
        for (char c : order.toCharArray()) {
            for (int i = 0; i < tokens.size(); i++) {
                if (!String.valueOf(c).equals(tokens.get(i))) continue;
                
                long pre = Long.parseLong(tokens.get(i - 1));
                long post = Long.parseLong(tokens.get(i + 1));
                long result = calculate(pre, post, c);
                tokens.remove(i - 1);
                tokens.remove(i - 1);
                tokens.remove(i - 1);
                tokens.add(i - 1, String.valueOf(result));
