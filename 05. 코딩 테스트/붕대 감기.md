...
tags:
  - 구현
date: 2025-12-24
복습 풀이: 251224(X), 251229(O), 260104(O)
---
# 1차 풀이
```java
...
class Solution {
    public int solution(int[] bandage, int health, int[][] attacks) {
        ...
        for (int[] attack : attacks) {
            int attackTime = attack[0];
            int dealValue = attack[1];
            
            int healValue = calculateHealValue(bandage, attackTime, currentTime);
            
            if (healValue + remainHealth >= health) {
                remainHealth = health;
            } else {
                remainHealth += healValue;
            }
            
            remainHealth -= dealValue;
    ...
    private int calculateHealValue(int[] bandage, int attackTime, int currentTime) {
        int castingTime = bandage[0];
        int healingPerSec = bandage[1];
        int additionalHealing = bandage[2];
        int diff = attackTime - currentTime;
        int time = diff / castingTime;
        
        if (time >= 1) return time * (castingTime * healingPerSec + additionalHealing) + ((diff % castingTime) * healingPerSec);
        return diff * healingPerSec;
    }
}
```