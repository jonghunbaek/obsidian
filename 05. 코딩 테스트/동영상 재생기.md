...
tags:
date: 2025-12-12
복습 풀이: 251212(X), 251220(O), 251229(O), 260104(O)
---
# 1차 풀이
```java
// 1. 시간 기반 구현 문제
...
class Solution {
    public String solution(String video_len, String pos, String op_start, String op_end, String[] commands) {
        ...
        position = moveOpeningEndIfInRange(position, openingStart, openingEnd);
        
        for (String command : commands) {
            if (command.equals("prev")) {
                position = position - 10 < 0 ? 0 : position - 10;    
            } else {
                position = position + 10 > videoLength ? videoLength : position + 10;
            }
    ...
    private int moveOpeningEndIfInRange(int position, int openingStart, int openingEnd) {
        if (openingStart <= position && position <= openingEnd) return openingEnd;
