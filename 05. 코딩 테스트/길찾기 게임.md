...
aliases:
  - 그래프
복습 풀이: 250909(X), 250913(X), 250920(O), 251004(O), 251010(O), 251017(O), 251024(O), 251106(O)
---
# 1차 풀이
```java
import java.util.*;
...
# 7차 풀이
```java
// 1. 주어진 노드 정보로 이진 트리를 만들기
// 2. 이진트리를 전위/후위 순회로 탐색해 결과 반환
import java.util.*;
...
class Solution {
    public int[][] solution(int[][] nodeinfo) {
        ...
        List<Integer> preorder = new ArrayList<>();
        List<Integer> postorder = new ArrayList<>();
        orderPre(nodes[0], preorder);
        orderPost(nodes[0], postorder);
