...
date: 2025-11-20
aliases:
복습 풀이: 251120(X), 251201(X), 251209(X), 251213(X), 251220(X), 251227(X), 260103(O), 260110(X), 260117(O)
---
# 1차 풀이
```java
// 1. 구현 
...
class Solution {
    public int[] solution(int n, int[] info) {
        int[] ryan = shot(0, n, new int[info.length], info);
        
        return ryan == null ? new int[] {-1} : ryan;
    }
    
    private int[] shot(int idx, int n, int[] hits, int[] apeach) {
        if (idx == hits.length) {
            if (n > 0) return null; // 왜? n이 0보다 커도 게임에서 이길 가능성이 있지 않을까?
            if (calculateDiff(hits, apeach) <= 0) return null;
            return Arrays.copyOf(hits, hits.length);
        }
        ...
        for (int i = 0; i <= n; i++) {
            hits[idx] = i;
            int[] ryan = shot(idx + 1, n - i, hits, apeach);
            ...
            if (diff > maxDiff) {
                maxDiff = diff;
                result = ryan;
            } else if (diff == maxDiff && isPrior(ryan, result)) {
                result = ryan;
            }
        }
    ...
    private boolean isPrior(int[] ryan, int[] result) {
        for (int i = 10; i >= 0; i--) {
...
class Solution {
    ...
    private int[] shot(int idx, int remain, int[] hits, int[] apeach) {
        if (idx == hits.length) {
            if (calculateDiff(hits, apeach) <= 0) return null;
            return hits.clone();
        }
        ...
        for (int count = 0; count <= remain; count++) {
            hits[idx] = count;
            int[] ryan = shot(idx + 1, remain - count, hits, apeach);
            ...
            if (diff > maxDiff) {
                maxDiff = diff;
                results = ryan;
            } else if (diff == maxDiff && isPrior(ryan, results)) {
                results = ryan;
            }
        }
    ...
    private boolean isPrior(int[] ryan, int[] results) {
        for (int i = 10; i >= 0; i--) {
            if (ryan[i] == results[i]) continue;
            
            return ryan[i] > results[i];
        }
...
class Solution {
    ...
    private void shot(int idx, int n, int[] ryan, int[] apeach, int[] result) {
        if (idx == apeach.length) {
            if (calculateScoreDiff(ryan, apeach) <= 0) {
                result = null;
                return;
            }
        ...
        for (int i = 0; i <= n; i++) {
            ryan[idx] = i;
            shot(idx + 1, n - i, ryan, apeach, result);
            
            if (result == null) {
                continue;
            }
            
            int diff = calculateScoreDiff(ryan, apeach);
            if (diff > maxDiff) {
                maxDiff = diff;
                result = ryan.clone();
            } else if (diff == maxDiff && isPrior(ryan, result)) {
    ...
    private int calculateScoreDiff(int[] ryan, int[] apeach) {
        int diff = 0;
        for (int i = 0; i < ryan.length; i++) {
            if (ryan[i] == 0 && apeach[i] == 0) continue;
...
```
기존 풀이의 재귀 함수 반환 타입을 void로 바꿔 풀이 해보려했으나 실패.
- 반복문에서 result = ryan.clone()을 하는 것은 주소 값 자체를 바꾸는 것이므로 solution()에서 생성한 result에는 영향x(대신 값을 직접 바꿔주는 것은 ok)
- 반환 타입이 있는 경우는 bottom-up 방식이 되어 항상 최선의 결과를 부모에게 전달하여 결과를도출
- 없는 경우엔 top-down 방식이 됨
...
class Solution {
    ...
    private int[] shot(int idx, int n, int[] hits, int[] apeach) {
        if (idx == hits.length) {
            if (countScoreDiff(hits, apeach) <= 0) return null;
            return hits.clone();
        }
        ...
        for (int i = 0; i <= n; i++) {
            ...
            int diff = countScoreDiff(ryan, apeach);
            if (diff > maxDiff) {
                maxDiff = diff;
                results = ryan;
            } else if (diff == maxDiff && isBetter(results, ryan)) {
                results = ryan;
            }
        }
    ...
    private int countScoreDiff(int[] hits, int[] apeach) {
        int diff = 0;
        for (int i = 0; i < hits.length; i++) {
            if (hits[i] == 0 && apeach[i] == 0) continue;
            if (hits[i] > apeach[i]) {
                diff += 10 - i;
            } else {
                diff -= 10 - i;
    ...
    private boolean isBetter(int[] origin, int[] target) {
        for (int i = 10; i >= 0; i--) {
            if (origin[i] == target[i]) continue;
            return origin[i] < target[i];
        }
