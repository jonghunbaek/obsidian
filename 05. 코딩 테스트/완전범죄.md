...
# 2차 풀이
```java
// 1. 1차 풀이에선 완전 탐색으로 접근해 시간초과로 실패
// 2. 1개 훔칠 때부터 최대 값인 40개 훔칠 때까지 모두 재귀로 탐색해서 최솟 값을 찾아야 하는데, 이 부분이 시간초과
// 3. info 최댓값이 40이기 때문에 시간 복잡도 계산은 다음과 같음
//  - 1개 일 때, 40번
//  - 2개 일 때, 40C2로 780
//  - 39개 일 때, 40C39으로 
// 4. 혹은 둘 중하나만 선택하는 과정이므로 2의 40승이 되기도 함(결과 값은 동일)

// 1. 따라서 dp로 접근해야 함
// 2. a도둑의 최솟값을 찾아야하므로 b도둑의 흔적 개수마다 최솟값을 찾아야함
import java.util.*;
