...
aliases:
  - 좌표 구현
복습 풀이: 250712(X), 250721(X), 250726(X), 250802(X), 250809(O), 251014(O)
---
# 1차 풀이
```java
import java.util.*;
...
# 6차 풀이
```java
// 1. 2중 반복문으로 모든 직선의 교점 구하기
// 2. 교점이 정수인 경우엔 List에 담기, 아닌 경우엔 null을 반환하여 담지 않기
//  2-1. x, y의 최솟,최댓 값 구하기
// 3. x,y의 최솟,최댓 값을 기준으로 문자열 배열 생성 후 .으로 fill
// 4. List를 순회하며 교점을 표시
